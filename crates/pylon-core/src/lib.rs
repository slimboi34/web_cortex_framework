//! Pylon core runtime.
//!
//! A Pylon application is declared in Python, compiled to a [`manifest::Manifest`],
...
pub fn init_tracing(default_level: &str) {
    ...
    let filter = EnvFilter::try_from_env("PYLON_LOG")
...
mod tests {
    ...
    async fn static_route_serves_without_an_interpreter() {
        ...
        let res = app.dispatch(PylonRequest::synthetic("GET", "/ping")).await;
    ...
    async fn unknown_path_404s_and_wrong_method_405s() {
        ...
        let app = App::build_without_python(m).await.expect("app builds");
        assert_eq!(app.dispatch(PylonRequest::synthetic("GET", "/nope")).await.status, 404);
        assert_eq!(app.dispatch(PylonRequest::synthetic("POST", "/ping")).await.status, 405);
    }
