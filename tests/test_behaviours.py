...
app = Pylon("beh", database="sqlite://./beh.db", port={port})
...
def beh_server():
    port = _free_port()
    workdir = Path(tempfile.mkdtemp(prefix="pylon-beh-"))
...
def test_control_plane_lists_behaviours(beh_server):
    names = {b["name"] for b in get(beh_server, "/_pylon/behaviours")["behaviours"]}
