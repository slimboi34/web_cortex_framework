...
def app_with_auth() -> Pylon:
    app = Pylon("sec", database="sqlite://:memory:")
    app.api_key("ADMIN_KEY", id="admin", scopes=["read", "write", "pylon:admin"])
...
def test_api_keys_are_referenced_by_env_var_never_embedded():
    ...
    assert "secret" not in serialized.lower() or "PYLON" in serialized
...
def test_security_headers_are_on_by_default():
    assert Pylon("t").manifest()["security_headers"]["enabled"] is True


def test_rate_limit_is_off_until_declared():
    assert Pylon("t").manifest()["rate_limit"]["enabled"] is False
    app = Pylon("t")
...
def test_page_cannot_take_two_data_sources():
    app = Pylon("t", database="sqlite://:memory:", templates="templates")
...
app = Pylon("sec", database="sqlite://./sec.db", port={port})
app.api_key("ADMIN_KEY", id="admin", scopes=["read", "write", "pylon:admin"])
app.api_key("READER_KEY", id="reader", scopes=["read", "pylon:admin"])
...
def secure_server():
    port = _free_port()
    workdir = Path(tempfile.mkdtemp(prefix="pylon-sec-"))
...
def test_control_plane_requires_admin_scope(secure_server):
    assert call(secure_server, "/_pylon/openapi.json")[0] == 401
    assert call(secure_server, "/_pylon/openapi.json", key=ADMIN)[0] == 200


def test_health_stays_reachable_without_a_credential(secure_server):
    """Load balancers cannot present an API key."""
    assert call(secure_server, "/_pylon/health")[0] == 200


def test_security_endpoint_reports_the_public_surface(secure_server):
    status, body, _ = call(secure_server, "/_pylon/security", key=ADMIN)
    assert status == 200
